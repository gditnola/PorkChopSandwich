...
@class FeatureGraphNode, FeatureGraphEdge;

/**
	Describes a complete route between two nodes in a FeatureGraph.  Not meant
    to be constructed directly from client programs, but instead managed
    by calls to methods like shortestRouteFromNode:toNode: in FeatureGraph
 */
@interface FeatureGraphRoute : NSObject {

    /**
        A collection of FeatureGraphRouteStep objects that describe a route
...
/**
    A collection of FeatureGraphRouteStep objects that describe a route
    between two nodes in a graph.  The first returned FeatureGraphRouteStep will
